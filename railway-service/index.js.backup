...
const { generatePairingCode, clearPairingCode } = require('./pairing-handler');
...
async function connectWhatsApp(device, isRecovery = false) {
  ...
  try {
    ...
    const sock = makeWASocket({
      ...
      mobile: false, // Important: set to false for web pairing
      pairingCode: isPairingMode ? true : undefined, // Enable pairing code mode
      phoneNumber: isPairingMode ? phoneForPairing : undefined, // Phone number for pairing
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      // ==========================================
      // REQUEST PAIRING CODE - For pairing mode
      // ==========================================
      if (isPairingMode && !hasValidSession && !isRecovery && !pairingCodeRequested) {
        ...
        if (!sock.authState.creds.registered && phoneForPairing) {
          ...
          console.log(`🔐 [${deviceName}] Requesting pairing code for: ${phoneForPairing}`);
          ...
          try {
            ...
            if (sock.requestPairingCode) {
              ...
              if (code) {
                console.log(`✅ [${deviceName}] Pairing code received: ${code}`);
              ...
              console.log(`⚠️ [${deviceName}] requestPairingCode not available, waiting for code in update...`);
            }
          } catch (err) {
            console.error(`❌ [${deviceName}] Error requesting pairing code:`, err);
            pairingCodeRequested = false;
          }
        }
      }
      
      // Handle pairing code in connection update (for some Baileys versions)
      if (update.pairingCode && isPairingMode && !hasValidSession) {
        const code = update.pairingCode;
        console.log(`📱 [${deviceName}] Pairing code from update: ${code}`);
